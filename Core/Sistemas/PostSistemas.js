export async function enviarSistemas(isEditing, sistema) {
    try {
        const config = useRuntimeConfig()
        const token = localStorage.getItem('token')
        const method = isEditing ? 'PUT' : 'POST'
        const url = isEditing ?
            `${config.public.api}/${config.public.sistema}/${sistema.id}` :
            `${config.public.api}/${config.public.sistema}`
        ...
        console.error('Error al enviar sistema:', error);
        throw error;
    }
}
